...
{
	$query = "INSERT INTO rutas";/* "rutas" es el nombre de la tabla en la base de datos */
	$query .= "(consenti, genero, municipio, comuna, barrio, experiencia, rutas, correo) ";/* el .= lo que hace es concatenar y en esa fila van los NOMBRES DE LAS COLUMNAS EN LA BASE DE DATOS */
	$query .= "VALUES(:preg0, :preg3, :preg4, :preg5_com, :preg5, :preg12,:preg13, :preg32);";/* AQUÍ SON LOS VALORES DE LAS VARIABLES EN LA ENCUESTA, QUE NO ESTÁN DEFINIDAS, LUEGO SE ASOCIAN A LOS VALORES DE LA ENCUESTA CON EL POST*/
	...
	if($statement->execute($user_data))
	{
		...
		$query = "UPDATE rutas SET rutas=:preg13 where id=:id";// (nombre en base de datos) = :(nombre de la columna a actualizar)** separada por ","
		$user_data = array(
			':preg13' => $rutas13,
			':id' => $newid
		);
